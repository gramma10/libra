...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-border bg-card shadow-apple-lg p-6 space-y-5">
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">{t("onboarding.creatorCode")}</label>
              <div className="relative">
                <Input type={showCode ? "text" : "password"} placeholder={t("onboarding.codePlaceholder")} value={creatorCode} onChange={(e) => { setCreatorCode(e.target.value); setCodeError(""); }} className="rounded-xl pr-10" autoFocus />
                <button type="button" onClick={() => setShowCode(!showCode)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
                  {showCode ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {codeError && <p className="text-sm text-destructive">{codeError}</p>}
              <p className="text-xs text-muted-foreground">{t("onboarding.codeHelp")}</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">{t("onboarding.shopName")}</label>
              <Input placeholder={t("onboarding.shopNamePlaceholder")} value={shopName} onChange={(e) => setShopName(e.target.value)} className="rounded-xl" />
            </div>
            <div className="space-y-2">
