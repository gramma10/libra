...
export default function SettingsPage() {
  ...
  const handleSendTestSms = async () => {
    if (!testPhone) { toast.error(t("settings.enterPhone")); return; }
    setSendingTest(true);
    ...
    else toast.success(t("settings.testSmsSent"));
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (data) {
        ...
        if (data.operating_hours) setOperatingHours(data.operating_hours as unknown as DayHours[]);
      }
      setLoading(false);
    };
    fetchSettings();
  ...
  const handleSave = async () => {
    ...
    if (invalid) {
      toast.error(`${invalid.day}: ${t("settings.closingAfterOpening")}`);
      return;
    }
    setSaving(true);
    const payload = { sms_enabled: smsEnabled, apifon_sender_id: apifonSenderId, operating_hours: operatingHours as any };
    if (settingsId) {
      const { error } = await supabase.from("business_settings").update(payload).eq("id", settingsId);
      if (error) toast.error(error.message); else toast.success(t("settings.settingsSaved"));
    } else {
      const { error } = await supabase.from("business_settings").insert({ ...payload, shop_id: shopId! });
      if (error) toast.error(error.message); else toast.success(t("settings.settingsCreated"));
    }
    setSaving(false);
  };
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6 max-w-5xl">
      ...
      <Tabs defaultValue="general" className="w-full">
        <TabsList className="rounded-xl mb-6">
          ...
          <TabsTrigger value="booking-theme" className="rounded-lg">{t("settings.bookingTheme")}</TabsTrigger>
        </TabsList>

        <TabsContent value="general" className="space-y-8 max-w-3xl">
          <Card className="rounded-2xl shadow-apple">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2"><Globe className="h-5 w-5" strokeWidth={1.5} />{t("settings.operatingHours")}</CardTitle>
              <CardDescription>{t("settings.operatingHoursDesc")}</CardDescription>
            </CardHeader>
            <CardContent><OperatingHoursEditor hours={operatingHours} onChange={setOperatingHours} /></CardContent>
          </Card>

          <Card className="rounded-2xl shadow-apple">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2"><MessageSquare className="h-5 w-5" strokeWidth={1.5} />{t("settings.smsReminders")}</CardTitle>
              <CardDescription>{t("settings.smsRemindersDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">{t("settings.enableSms")}</p>
                  <p className="text-xs text-muted-foreground">{t("settings.enableSmsDesc")}</p>
                </div>
                <Switch checked={smsEnabled} onCheckedChange={setSmsEnabled} />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">{t("settings.apifonSenderId")}</label>
                <Input value={apifonSenderId} onChange={(e) => setApifonSenderId(e.target.value)} className="rounded-xl max-w-sm" placeholder="SALON" />
                <p className="text-xs text-muted-foreground">{t("settings.senderIdDesc")}</p>
              </div>
              <div className="border-t border-border pt-4 space-y-2">
                <label className="text-sm font-medium">{t("settings.sendTestSms")}</label>
        ...
        <TabsContent value="booking-theme"><BookingThemeEditor /></TabsContent>
      </Tabs>
