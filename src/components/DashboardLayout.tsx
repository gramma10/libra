...
import {
  ...
  Sparkles, UserCog, Receipt, User, LogOut, Globe, Menu,
} from "lucide-react";
  ...
  const sidebarContent = (
    <>
      ...
      <nav className="flex flex-1 flex-col gap-1 p-3 overflow-y-auto">
        {navItems.map((item) => (
          <div key={item.to} onClick={() => isMobile && setMobileOpen(false)}>
            <SidebarNavItem {...item} collapsed={false} />
          </div>
        ))}
      </nav>
      ...
      <div className="border-t border-border/50 p-3 space-y-1">
        <button
          ...
          className="flex w-full items-center gap-3 rounded-xl p-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
        >
        ...
        <button
          onClick={signOut}
          className="flex w-full items-center gap-3 rounded-xl p-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive"
        >
  ...
  if (isMobile) {
    return (
      <div className="flex min-h-screen w-full flex-col">
        {/* Mobile top bar */}
        <header className="glass-strong sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-border/50 px-4">
